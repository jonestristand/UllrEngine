...
namespace Ullr {
  ...
  void MacOSWindow::Init(const WindowProps& props)
  {
    ...
    glfwSetWindowSizeCallback(this->glfwWindow, [](GLFWwindow* window, int width, int height) {
      macosWindowData& data = *(macosWindowData*)glfwGetWindowUserPointer(window);
  ...
  void MacOSWindow::setWindowMode(const WindowMode & mode, unsigned int width = 0, unsigned int height = 0)
  {
